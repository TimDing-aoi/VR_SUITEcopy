...
{
    [InitializeOnLoad]
    public abstract class PlasticDialog : EditorWindow, IPlasticDialogCloser
    {
        ...
        void OnDestroy()
        {
            ...
            if (!mIsCompleted)
            {
                var cloneModal = CloneModal();

                EditorDispatcher.Dispatch(() =>
                {
                    cloneModal.RunModal(mParentWindow);
                });
            }
            else
            {
        ...
        protected abstract PlasticDialog CloneModal();
