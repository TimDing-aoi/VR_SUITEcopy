...
namespace Codice.Views.IncomingChanges.Gluon
{
    internal class IncomingChangesTreeView : TreeView
    {
        ...
        static GetOverlayIcon.Data GetOverlayData(
            IncomingChangeInfo incomingChange,
            bool isSolvedConflict)
        {
            if (incomingChange.CategoryType == IncomingChangeCategory.Type.Conflicted)
                return GetOverlayIcon.ForConflict(isSolvedConflict);

            return GetOverlayIcon.ForChangeToApply(incomingChange.IsXLink());
        }
